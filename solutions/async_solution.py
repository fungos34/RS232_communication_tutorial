...
class BKPrecisionRS232():
	...
	* 'OUT OFF\r'             	deactivates power output
	...
	async def send_encoded_command(self, encoded_command) -> None:
		...
		# logger.info(f"SENDING >>> '{encoded_command.decode('ascii')}' ")
	...
	async def receive_one_byte(self) -> bytes:
		...
		# logger.info(f"RECEIVING <<< '{received.decode('ascii')}' ")
	...
	def verify_response(self, initial_command: str, formatted_response: str) -> bool:
		if '?' not in initial_command:
			...
			if formatted_response == '':
				...
				return False
		else: # elif initial_command in ['IDN?','STAT?','VOLT?','CURR?']:
			if formatted_response != '':
				return True
			else:
				return False
...
async def main():
	...
	bkp = BKPrecisionRS232('COM4')
	await bkp.initialize_connection()
	await asyncio.gather(bkp.start_monitoring(time_interval=1), bkp.stop_monitoring(delay=10))
	await bkp.close_port()


if __name__ == '__main__':
	# asyncio.run(bkp_test_communication())
	asyncio.run(main())