...
class VirtualDevice():
    ...
    """
    def __init__(self, virtual_port: str = 'COM14', baudrate: int = 19200) -> None: #response_register: dict = {},
    ...
    async def update_voltage(self, state):
        logger.info(f'updating voltage state to {state}')
        self.voltage = state
        self.update_response_register()

    async def update_current(self, state):
        logger.info(f'updating current state to {state}')
        self.current = state
        self.update_response_register()

    async def update_status(self, state):
        logger.info(f'updating status to {state}')
        self.status = state
        self.update_response_register()

    async def respond(self, response):
        if type(response) == str and response == '':
            logger.info(f'response is an empty string.')
            ...
            logger.info(f'response was a string.')
        ...
        if type(response) == bytes:
            logger.info(f'response was a byte: {response}')
            self._writer.write(response)

    async def eavesdropper(self):
        try:
            logger.info(f'starts eavesdropping ... process ID: {os.getpid()}')
    ...
    async def run_eavesdropper(self):
        while True:
            ...
            if inbox in self.response_register.keys():
                ...
                logger.info(inbox.decode('ascii').strip())
...
async def main(virtual):
    ...
    logger.info(f'##################\nport {virtual.virtual_port} open: {virtual.port_open}\n##################')
...
if __name__ == '__main__':
    try:
        vrtl = VirtualDevice()#valid_response_register)
